...
void template_filter(volatile uint32_t* in_data, volatile uint32_t* out_data, dimensions* cookieX, dimensions* cookieY, dimensions* cookieWidth, dimensions* elmoX,dimensions* elmoY, dimensions* elmoWidth){
	...
	pixel_type prev_green = 0;
	pixel_type prev_red = 0;
	pixel_type prev_blue = 0;

	dimensions avg_red;
	dimensions avg_green;
	dimensions avg_blue;
	...
	for (dimensions y = 0; y < h; ++y) {
		...
		for (dimensions x = 0; x < w; ++x) {
			...
			if(x % 25 == 8) { // initialization for prev average
				avg_red = in_red;
				avg_green = in_green;
				avg_blue = in_blue;
			} else if(x % 25 == 4) { //initialization for in average
				prev_red = avg_red / 4;
				prev_green = avg_green / 4;
				prev_blue = avg_blue / 4;

				avg_red = in_red;
				avg_green = in_green;
				avg_blue = in_blue;
			} else if (x % 25 == 0) { //change .widths as well

				in_red = avg_red / 4;
				in_green = avg_green / 4;
				in_blue = avg_blue / 4;
				...
				if(cookieStart) {
					...
					if(in_red>135 /*&& (in_red-in_blue)<170 && (in_red-in_blue)>60 && (in_red-in_green)<170 && (in_red-in_green)>60 && prev_blue<40 && prev_red<40 && prev_green<40*/) {
						if(counter * 25 > *elmoWidth) {
